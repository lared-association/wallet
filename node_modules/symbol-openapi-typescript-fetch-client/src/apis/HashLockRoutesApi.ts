...
export class HashLockRoutesApi extends runtime.BaseAPI {
    ...
    async searchHashLockRaw(requestParameters: SearchHashLockRequest): Promise<runtime.ApiResponse<HashLockPage>> {
        if (requestParameters.address === null || requestParameters.address === undefined) {
            throw new runtime.RequiredError('address','Required parameter requestParameters.address was null or undefined when calling searchHashLock.');
        }
        ...
        const response = await this.request({
            path: `/account/{address}/lock/hash`.replace(`{${"address"}}`, encodeURIComponent(String(requestParameters.address))),
            method: 'GET',
            headers: headerParameters,
            query: queryParameters,
        });
